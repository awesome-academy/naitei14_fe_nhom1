...
export const RevenueChart = ({
  ...
  const stats = useMemo(() => {
    ...
    const recent =
      recentSlice.reduce((sum, val) => sum + val, 0) /
      Math.max(recentSlice.length, 1);
    const previousAvg =
      previousSlice.length > 0
        ? previousSlice.reduce((sum, val) => sum + val, 0) /
          previousSlice.length
        : 0;
    const trend =
      previousAvg > 0 ? ((recent - previousAvg) / previousAvg) * 100 : 0;
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex bg-gray-100 rounded-lg p-1">
            ...
            >
              <FaCalendarWeek className="mr-2" />7 ngày
            </button>
            <button
