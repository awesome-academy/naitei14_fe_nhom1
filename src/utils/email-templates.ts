import { EmailTemplate, EmailTemplateType, WelcomeEmailData, OrderConfirmationEmailData, ResetPasswordEmailData, ContactFormEmailData } from '@/src/types/email';
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>Cảm ơn bạn đã tin tướng DrinkShop!</p>
...
const getWelcomeTemplate = (data: WelcomeEmailData): EmailTemplate => {
  const content = `
    <h2>Chào mừng ${data.name} đến với DrinkShop! 🎉</h2>
    ...
    <a href="${data.verificationLink}" class="button">Xác thực Email</a>
    ` : ''}
...
const getResetPasswordTemplate = (data: ResetPasswordEmailData): EmailTemplate => {
  const content = `
    ...
    <ul>
        <li>Link này sẽ hết hạn vào ${data.expirationTime}</li>
...
const getOrderStatusUpdateTemplate = (data: { orderNumber: string; customerName: string; status: 'confirmed' | 'rejected'; reason?: string; }): EmailTemplate => {
  const content = `
    <h2>Cập nhật trạng thái đơn hàng #${data.orderNumber} 📦</h2>
    <p>Xin chào ${data.customerName},</p>
    <p>Đơn hàng của bạn đã <strong>${data.status === 'confirmed' ? 'được xác nhận' : 'bị từ chối'}</strong>.</p>
    ${data.status === 'rejected' && data.reason ? `<p><strong>Lý do từ chối:</strong> ${data.reason}</p>` : ''}
    <p>Cảm ơn bạn đã mua sắm tại DrinkShop!</p>
  `;
...
const getMonthlyRevenueReportTemplate = (data: { month: string; totalRevenue: number; topProducts: { name: string; revenue: number; }[]; }): EmailTemplate => {
  const topProductsHtml = data.topProducts.map(product => `
    <li>${product.name}: ${product.revenue.toLocaleString('vi-VN')}₫</li>
  `).join('');

  const content = `
    <h2>Báo cáo doanh thu tháng ${data.month} 📊</h2>
    <p>Xin chào,</p>
    <p>Doanh thu tháng ${data.month} của bạn là <strong>${data.totalRevenue.toLocaleString('vi-VN')}₫</strong>.</p>
