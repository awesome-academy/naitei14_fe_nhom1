...
// Popover và style import giữ nguyên nhưng Popover component đã được loại bỏ logic API
import { Popover, PopoverTrigger } from "@radix-ui/react-popover";
// Giữ import NotificationsPopover để giữ cấu trúc, nhưng component này sẽ cần được tối giản hoặc thay thế bằng placeholder nếu không có code
// import NotificationsPopover from "@/src/components/notification/NotificationsPopover"; // Comment hoặc xóa nếu không có code cho nó
import styles from "./MainNavbar.module.css";

// Định nghĩa menuItems giữ nguyên
const menuItems = [
  {
    label: "TRANG CHỦ",
    href: "/",
...
const NotificationsPopoverPlaceholder = ({ unreadCount, loading }) => {
  return (
    <div
...
const MainNavbar = () => {
  ...
  // Giá trị placeholder cho các state liên quan đến API
  const unreadCount = 2; // Số thông báo chưa đọc mẫu
  const cartItemCount = 3; // Số lượng sản phẩm trong giỏ hàng mẫu

  // Hàm placeholder cho Popover Notification (loại bỏ markAllAsSeen)
  const handleOpenChange = (open: boolean) => {
    // Logic khi Popover thay đổi trạng thái (chỉ giữ lại Popover cơ bản)
  };
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between py-3 lg:py-4">
          ...
          <div className="flex items-center space-x-2 lg:space-x-4">
            ...
            {/* Notifications Popover (giữ UI nhưng dùng placeholder) */}
            <div className="relative">
              {/* Giữ Popover để không thay đổi cấu trúc UI */}
              <Popover onOpenChange={handleOpenChange}>
                ...
                {/* Sử dụng placeholder hoặc loại bỏ nếu không muốn Popover */}
                {/* <NotificationsPopoverPlaceholder unreadCount={unreadCount} loading={false} /> */}
              </Popover>
              {unreadCount > 0 && (
            ...
            <Link href="/cart" className="relative">
              ...
              <Badge className="absolute -top-2 -right-2 bg-yellow-500 text-black text-xs">
                {cartItemCount} {/* Dùng giá trị placeholder */}
              </Badge>
            </Link>
