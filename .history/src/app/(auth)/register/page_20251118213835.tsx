...
// C067: Retaining as a constant, should ideally be in a config file
...
export default function RegisterUserPage() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      setServerError(null);
      // S006: Do not pass the password in the data object for logging or mock API calls
      const { password, confirmPassword, ...registrationData } = data;

      await handleUserRegistration({
        email: registrationData.email,
        // When sending to a real API, the password must be included here, but typically handled via a service.
        // For the S006 rule, we exclude sensitive data from this exposed object:
